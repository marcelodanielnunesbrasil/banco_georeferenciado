<?php
/***
 * @author (Henrique Arthur)[https://github.com/henriquearthur]
 * 
*/
class Database extends PDO
{
    ...
    public function getWhereValueFrom($column, $table, $where_column_key, $where_column_value)
    {
        $sql = $this->prepare("SELECT {$column} FROM {$table} WHERE {$where_column_key} = ?");

        $sql->bindValue(1, $where_column_value);
        $sql->execute();
    ...
    public function getWhereValueFromLike($column, $table, $where_column_key, $where_column_value)
    {
        $sql = $this->prepare("SELECT {$column} FROM {$table} WHERE {$where_column_key} LIKE ?");
